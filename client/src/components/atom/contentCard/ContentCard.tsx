...
type ContentCardProps = {
  ...
  setIgnoreChecked: React.Dispatch<React.SetStateAction<boolean>>;
};
...
export function ContentCard({
  ...
  const handleSingleCheck = (seq: number, event: React.MouseEvent) => {
  ...
  const handleOnlyCheck = (seq: number) => {
    // 선택된 seq만 isChecked가 되도록 설정
    setIsChecked(true);
    // 선택된 seq만으로 이루어진 배열을 setCheckList에 설정
    setCheckList([seq]);
    setIsEnabled(false);
  };
  ...
  return (
    <div>
      <Component>
        <Wrapper
          $isChecked={isChecked}
          onClick={() => handleOnlyCheck(content.contentSeq)}
        >
          <IconWrapper>
            <CheckBox
              isChecked={isChecked}
              onClick={(e) => handleSingleCheck(content.contentSeq, e)}
            ></CheckBox>
            {isChecked ? (
              <div>
                <svg
