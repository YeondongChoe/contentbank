//(function () {
    ...
    function Sketchpad(el, opts) {
        ...
        opts.width = opts.width || el.getBoundingClientRect().width;
        //opts.height = opts.height || opts.width * opts.aspectRatio;
        opts.height = window.innerHeight-document.getElementById("main_bt_wrap").getBoundingClientRect().height;
        ...
        function clearCanvas () {
            ...
            if (opts.backgroundColor) {
	            ...
	            if(document.querySelector("#mjx-editing-area").childNodes.length==0){
        ...
        function startLine (e) {
            e.preventDefault();
            //필기드로잉을 하면 그래프 설정 초기화와 울프람알파 데이터 초기화
	        document.getElementById("eq_box").setAttribute("data-state", "start");	
			wolfram_rst="";
    ...
    Sketchpad.prototype.resize = function (width) {
		...
		console.log(this.opts.width/this.opts.height);*/

        //var height = width * this.opts.aspectRatio;
        /*var height = window.innerHeight-document.getElementById("main_bt_wrap").getBoundingClientRect().height;
        //this.opts.lineSize = this.opts.lineSize * (width / this.opts.width);
		
		document.getElementById("img_box").style.height=height+"px";
		document.getElementById("eqn_wrap").style.height=height+"px";
		document.getElementById("ui_wrap").style.height=height+"px";
		document.getElementById("tab_hand_block").style.height=(height+20)+"px";
		document.getElementById("tab_img_block").style.height=(height+20)+"px";
